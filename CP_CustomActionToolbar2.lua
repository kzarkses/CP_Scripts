...
local extname = "CP_CUSTOM_ACTION_TOOLBAR 2"
...
-- Create context
local ctx = r.ImGui_CreateContext('Custom Action Toolbar 2')
...
function DisplayToolbarWidget()
    ...
    -- Start the window
    local visible, open = r.ImGui_Begin(ctx, 'Custom Action Toolbar 2', true, toolbar_flags)
...
function MainLoop()
    ...
    if not ctx or r.ImGui_ValidatePtr(ctx, "ImGui_Context*") == false then
        ctx = r.ImGui_CreateContext('Custom Action Toolbar 2')
