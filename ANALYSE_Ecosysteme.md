...
| 8 | ◐ **Sync BPM du Sampler FAIT 2026-07-22** : repitch vinyle via TUNE (`Kit.ApplyTempoSync`, ±80 st, garde du tune manuel), source BPM stockée en P_EXT + devinée du nom de fichier, menu pad « Sync to project tempo », ré-application sur changement de tempo (Engine/Tempo) ou de kit. **Reste : la quantisation de lancement dans `CP_MidiLooper`** (machine pending-launch côté JSFX). | Débloque les clips audio sans nouveau moteur. |
