...
| 3 | ✅ **FAIT 2026-07-22** (les briques : `CP_TempoHub.jsfx` gmem `CP_Tempo` + `Engine/Tempo.lua` avec fallback API, `NextBeatTime`/`NextBarTime` pour le start-on-beat et la quantisation de lancement, estimateur MIDI clock 24 ppqn scaffoldé). Les consommateurs se branchent aux chantiers suivants ; ⚠ MIDI matériel → Monitoring FX toujours à vérifier (cells 10/11). | Débloque le sync BPM du Sampler et unifie deux protocoles gmem. |
| 4 | ✅ **FAIT 2026-07-22.** `Engine/Tracks.lua` : dossier CP + marque `P_EXT:CP` (`Tracks.Ensure/NewChild`, adoption sans réarrangement) ; le parent kit et le routeur looper naissent dedans ; la profondeur de dossier de Kit est généralisée pour l'imbrication. (Le fallback par nom de Kit avait déjà disparu — découverte P_EXT + adoption RS5K.) | Règle l'usine à gaz, visible immédiatement. |
| 5 | ✅ **FAIT 2026-07-22.** `Engine/Clip.lua` : le descripteur (§4, avec `kind="midi"` portant les notes du modèle Roll) + sérialisation versionnée `CPC1\|k=v\|…` (%-escape, champs inconnus ignorés en lecture → forward-compatible). Pur Lua, zéro API. | Prérequis de tout le reste. |
| 6 | ✅ **FAIT 2026-07-22** — en deux temps : la persistance brute avait déjà atterri le 21/07 (blob v2 dans `P_EXT:CP_LOOPER_DATA`, autosave débouncé + Recall), AVANT l'exécution de ce plan ; ce chantier a livré le reste : `Loop.LaneToClip` / `Loop.ClipToLane`, les adaptateurs qui font d'une lane un Clip MIDI (le blob v2 testé reste le format de stockage). | Petit, et rend le concept de session réel. |
| 7 | ✅ **FAIT 2026-07-22.** `Engine/Bus.lua` : couche mince sur DragBus (qui reste domain-free dans le toolkit) — `BeginClip`/`TakeDrop` transportent des Clips CPC1, les drops `file` hérités sont PROMUS en Clip audio (un seul chemin de code côté consommateur), et `Send`/`Recv` généralisent l'idiome ExtState timestampé+TTL des ponts point-à-point. Les ponts eux-mêmes se branchent au fil des chantiers suivants. | Les ponts deviennent triviaux à ajouter. |
| 8 | ✅ **FAIT 2026-07-23** (en deux temps). 22/07 : sync BPM du Sampler — repitch vinyle via TUNE (`Kit.ApplyTempoSync`, ±80 st, garde du tune manuel), source BPM en P_EXT + devinée du nom de fichier, menu pad « Sync to project tempo », ré-application sur changement de tempo (Engine/Tempo) ou de kit. 23/07 : **launch quantize** dans `CP_MidiLooper` (LAYOUT_VER 6) — gmem[6] = pas de quantize en beats (Lua), LANE_STATE +6/+7 = action en attente + beat cible ; PLAY/STOP/REC/STOP-REC se mettent en file et tirent au premier bloc passé la frontière (maths de phase sur la CIBLE, donc calé grille même un bloc en retard) ; re-clic = annulation ; l'arm (mode 4) se convertit en rec quantisé ; CLEAR/PANIC/stop transport vident la file ; bouton Q (Off/beat/bar/2/4) + statut clignotant `> PLAY/STOP/REC/END` côté UI ; blob de sauvegarde v3 (launchq dans le bloc global, v1/v2 toujours lus). | Débloque les clips audio sans nouveau moteur. |
| 9 | ✅ **FAIT 2026-07-23.** `Engine\Bake.lua`, trois étages : (1) écrivain WAV pur Lua streaming (RIFF LE, 32-bit float par défaut / 16-bit PCM, header patché à la fermeture, `string.pack` par lots) ; (2) `RegionToWav` (une take telle qu'elle joue, via AudioAccessor, format source par défaut) et `FileRegionToWav` (un FICHIER garé sur une piste jetable le temps de la lecture — le « drag region out » d'Edison) ; (3) wrappers sélection-safe des rendus natifs Apply-FX 40209 / Glue 40362. Premier consommateur : menu pad Sampler « Bake: crop to trim » → nouveau WAV à côté de la source + `Kit.LoadSample` (offsets remis à zéro). | Keystone : édition destructive + boucles calées + enregistrement audio. |
