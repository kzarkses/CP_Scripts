...
id="",name="",is_enabled=true,window_to_follow="main",docker_id=0,snap_to="right",
use_responsive_layout=true,offset_x=10,offset_y=4,widget_width=330,widget_height=36,
last_pos_x=1000,last_pos_y=1000,font_size=16,current_font="Verdana",use_high_dpi_font=true,
background_color=0x1E1E1EFF,text_color=0xFFFFFFFF,button_color=0x1E1E1EFF,
button_hover_color=0x363636FF,button_active_color=0x1E1E1EFF,border_color=0x313131FF,
border_size=2.0,window_rounding=20.0,frame_rounding=20.0,popup_rounding=6.0,
grab_rounding=6.0,grab_min_size=8.0,button_border_size=1.0,button_spacing=0,
show_background=true,actions={},button_width=30,button_height=28,icon_size=30,
preserve_icon_aspect_ratio=true,show_icons=true,center_buttons=true,icon_path="",
show_tooltips=true,use_reaper_icons=true,min_window_width=1800,min_window_height=200,
auto_hide=true,adaptive_width=50,settings_open=false,section_states={position=true,
appearance=false,buttons=false,actions=true,import_export=false},context=nil,font=nil,
font_needs_update=false,force_position_update=false,first_position_set=false,
icons={},reaper_icon_states={},last_window_check=0,cached_target_rect=nil
}
...
if type(k)=="string"then res=res..'["'..k..'"]='
else res=res.."["..k.."]="end
if type(v)=="table"then res=res..r.serialize(v)
elseif type(v)=="string"then res=res..string.format("%q",v)
elseif type(v)=="number"or type(v)=="boolean"then res=res..tostring(v)end
res=res..","
end
return res.."}"
end
...
tb.snap_to="left"
elseif not tb.window_to_follow then tb.window_to_follow="main"end
tb.context=nil
tb.font=nil
...
if success then global_pushed_colors,global_pushed_vars=colors,vars end
end
...
actions[idx]={command_id=tonumber(command_id),name=command_name,
label=command_name:match("[^:]+$")or command_name,icon=icons[idx]or""}
else
actions[idx]={command_id=command_id,name=command_name,
label=command_name:match("[^:]+$")or command_name,icon=icons[idx]or""}
end
end
end
...
"Import Actions",1)==1
if replace then toolbar.actions=imported_actions
else
...
local manager_flags=r.ImGui_WindowFlags_NoTitleBar() | r.ImGui_WindowFlags_NoResize() | r.ImGui_WindowFlags_NoCollapse()
...
r.ImGui_PushID(main_ctx,i)
if i>1 then r.ImGui_Spacing(main_ctx)r.ImGui_Spacing(main_ctx)end
if action.is_separator then
...
if r.ImGui_Button(main_ctx,"Down##down"..i,50,22)and i<#tb.actions then item_to_move_down=i end
r.ImGui_SameLine(main_ctx,0,8)
r.ImGui_PushStyleColor(main_ctx,r.ImGui_Col_Button(),0xB34C4CFF)
...
if font_pushed then r.ImGui_PopFont(main_ctx)end
if style_loader then
...
InitMainFont()
if style_loader then style_loader.applyFontsToContext(main_ctx)end
