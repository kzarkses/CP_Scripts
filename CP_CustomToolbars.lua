    ...
    if state_str ~= "" then
        ...
        if success and toolbar_list and #toolbar_list > 0 then
            ...
            for _, tb_info in ipairs(toolbar_list) do
                ...
                -- Backward compatibility for screen_edge_mode conversion to snap_to
                if tb.screen_edge_mode and not tb.snap_to then
                    if tb.screen_edge_mode == "left" then
                        tb.snap_to = "left"
                    else
                        tb.snap_to = "right"
                    end
...
-- Create REAPER action for a toolbar
-- Create REAPER action for a toolbar
...
function GetCommandName(command_id)
    ...
    -- Pour les commandes numériques, utiliser CF_GetCommandText si disponible
    if type(command_id) == "number" then
        -- Essayer d'abord avec CF_GetCommandText (extension SWS)
        if r.CF_GetCommandText then
            local name = r.CF_GetCommandText(0, command_id) -- 0 = section principale
            if name and name ~= "" then
                return name
            end
        end
        
        -- Si pas de nom trouvé, simplement retourner l'ID
        return "Command ID: " .. tostring(command_id)
    else
        -- Pour les commandes nommées, essayer de les rechercher
        local cmd_id = 0
        pcall(function() cmd_id = r.NamedCommandLookup(command_id) end)
        
        if cmd_id and cmd_id ~= 0 then
            -- Si on a pu obtenir un ID, essayer CF_GetCommandText
            if r.CF_GetCommandText then
...
function BrowseIconsDialog(toolbar, action_index)
    -- Si le contexte principal n'est pas valide, ne rien faire
    ...
    if r.ImGui_BeginPopupModal(main_ctx, "Icon Browser", true) then
        -- Chemin par défaut si aucun n'est défini
        local base_path = toolbar.icon_path or ""
        if base_path == "" then
            base_path = r.GetResourcePath() .. "/Data/toolbar_icons/"
        end
        ...
        -- Créer un ScrollingArea pour les icônes
        if r.ImGui_BeginChild(main_ctx, "IconGrid", -1, -40) then
            -- Lister les fichiers avec filtrage
            local files = {}
            local i = 0
            local file = r.EnumerateFiles(base_path, i)
            while file do
                if file:match("%.png$") or file:match("%.ico$") then
                    if static_filter == "" or file:lower():find(static_filter:lower(), 1, true) then
            ...
            -- Affichage des icônes en grille manuelle
            local current_col = 0
            for _, file in ipairs(files) do
                -- Si nous ne sommes pas à la première colonne, utiliser SameLine
                if current_col > 0 then
                    r.ImGui_SameLine(main_ctx)
                end
                
                -- Essayer de charger l'icône
                local texture = LoadIcon(toolbar, file)
                local selected = false
                
                -- Afficher soit un bouton image, soit un bouton texte
                if texture and r.ImGui_ValidatePtr(texture, "ImGui_Image*") then
                    selected = r.ImGui_ImageButton(main_ctx, file, texture, icon_size, icon_size)
                else
                    selected = r.ImGui_Button(main_ctx, file, icon_size, icon_size)
                end
                
                -- Infobulles avec nom de fichier
                ...
                -- Si l'icône est sélectionnée
                if selected then
                    -- Appliquer l'icône à l'action actuelle
                    if toolbar.actions[action_index] then
                        toolbar.actions[action_index].icon = file
                        -- Vider les caches d'icônes pour forcer le rechargement
...
function GetCommandName(command_id)
    ...
    if type(GetCommandName) ~= "function" then
    ...
    -- Utiliser pcall pour capturer les erreurs potentielles
    if type(command_id) == "number" then
        local success, name = pcall(function()
            local _, cmd_name = GetCommandName(command_id)
            return cmd_name
        end)
        ...
        -- Pour les commandes nommées
        local success, cmd_id = pcall(function() 
            return r.NamedCommandLookup(command_id)
        end)
        
        if success and cmd_id and cmd_id ~= 0 then
            local name_success, name = pcall(function()
                local _, cmd_name = GetCommandName(cmd_id)
                return cmd_name
            end)
            
            if name_success and name and name ~= "" then
                return name
            end
        end
...
function DisplayToolbarWidget(toolbar)
    ...
    -- Get REAPER's main window dimensions
    local main_hwnd = r.GetMainHwnd()
    local retval, LEFT, TOP, RIGHT, BOT = r.JS_Window_GetRect(main_hwnd)
    ...
    -- Set position based on snap option
    if toolbar.snap_to == "left" then
    ...
    elseif toolbar.snap_to == "right" then
    ...
    if visible then
        ...
        if toolbar.snap_to ~= "left" and toolbar.snap_to ~= "right" then
    ...
    r.ImGui_EndChild(main_ctx)  -- Important: This should only be called if BeginChild succeeded
        ...
        if r.ImGui_Button(main_ctx, "Create Actions", 100, 24) and current_toolbar_index <= #toolbars then
            ...
            if result.success then
                ...
                local messages = {}
                -- Vérifions quel problème s'est produit
                if result.on_id ~= 0 and result.off_id ~= 0 then
                    -- Les actions existent mais il y a eu un problème
        ...
        if current_toolbar_index <= #toolbars then
            ...
            if r.ImGui_CollapsingHeader(main_ctx, "Position Settings", r.ImGui_TreeNodeFlags_DefaultOpen()) then
                ...
                -- Position mode (Radio buttons for snap to left/right)
                r.ImGui_Text(main_ctx, "Position Mode:")
                
                local is_left = tb.snap_to == "left"
                rv = r.ImGui_RadioButton(main_ctx, "Snap to Left Edge", is_left)
                if rv and not is_left then
                    tb.snap_to = "left"
                    tb.force_position_update = true
                    tb.first_position_set = false
                end
                ...
                r.ImGui_SameLine(main_ctx)
                local is_right = tb.snap_to == "right"
                rv = r.ImGui_RadioButton(main_ctx, "Snap to Right Edge", is_right)
                if rv and not is_right then
                    tb.snap_to = "right"
                    tb.force_position_update = true
                    tb.first_position_set = false
                end
            ...
            if r.ImGui_CollapsingHeader(main_ctx, "Import/Export") then
                ...
                -- REAPER Action Management
                r.ImGui_Text(main_ctx, "REAPER Action Management:")
                if r.ImGui_Button(main_ctx, "Create/Update REAPER Action") then
                    local action_id = CreateToolbarAction(tb)
                    if action_id ~= 0 then
                        r.ShowMessageBox("REAPER action created successfully: CP_CustomToolbars_" .. tb.name, "Success", 0)
                    end
                end
            end
                ...
                if r.ImGui_Button(main_ctx, "Add Action") then
                    ...
                    if retval and retvals ~= "" then
                        ...
                        if command_id then
                            ...
                            local _, name = GetCommandName(command_id)
                            ...
                            if cmdId and cmdId ~= 0 then
                                -- Valid named command
                                local _, name = GetCommandName(cmdId)
                                table.insert(tb.actions, {
                                    command_id = retvals,
                        ...
                        for i, action in ipairs(tb.actions) do
                            ...
                            if cmd_changed then
                                ...
                                if num_cmd then
                                    ...
                                    local _, name = GetCommandName(num_cmd)
                                    if name ~= "" then 
                                        action.name = name
                                    end
                                else
                            ...
                            if r.ImGui_Button(main_ctx, "Browse...##" .. i, 80, 22) then
                                if r.APIExists("JS_Dialog_BrowseForOpenFiles") then
                                    ...
                                    -- Utiliser le navigateur d'icônes personnalisé
                                    r.ImGui_OpenPopup(main_ctx, "Icon Browser")
                                    current_editing_action_index = i
                                end
                            end
                            
                            BrowseIconsDialog(tb, current_editing_action_index)
...
-- Main loop
function MainLoop()
    -- Check if toolbar states have changed due to REAPER actions
...
function Init()
    ...
    -- Indiquer que le script est en cours d'exécution
...
function Exit()
    ...
    -- Indiquer que le script n'est plus en cours d'exécution
