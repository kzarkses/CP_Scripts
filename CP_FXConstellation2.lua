...
local min_val, max_val = r.TrackFX_GetParamEx(state.track, fx, param)
...
if not isTrackValid()then return false end
local current_time=r.time_precise()
if current_time-state.last_update_time<0.01 then
return false
end
...
local current_raw_value=r.TrackFX_GetParam(state.track,actual_fx_param_id,param_id)
local current_norm_value = (current_raw_value - param_data.min_val) / (param_data.max_val - param_data.min_val)
if math.abs(param_data.current_value-current_norm_value)>0.001 then
param_data.current_value=current_norm_value
if not param_data.selected then
param_data.base_value=current_norm_value
end
end
end
end
...
if param_data then
local scaled_value = param_data.min_val + new_value * (param_data.max_val - param_data.min_val)
...
r.TrackFX_SetParam(state.track,actual_fx_id,param_id,scaled_value)
...
local up_range,down_range=calculateAsymmetricRange(base_value,param_range,state.gesture_range,0,1)
    ...
    if state.fx_data[fx_id] and state.fx_data[fx_id].full_name == fx_preset.name then
      ...
      for param_id, param_preset in pairs(fx_preset.params or {}) do
        if state.fx_data[fx_id].params[param_id] and 
          ...
          local actual_fx_id = state.fx_data[fx_id].actual_fx_id or fx_id
          local scaled_value = state.fx_data[fx_id].params[param_id].min_val + param_preset.current_value * (state.fx_data[fx_id].params[param_id].max_val - state.fx_data[fx_id].params[param_id].min_val)
          r.TrackFX_SetParam(state.track, actual_fx_id, param_id, scaled_value)
...
function drawPresetsWindow()
  ...
  if state.preset_name == "" then
    if state.save_fx_chain then
      state.preset_name = "Chain1"
    else
      state.preset_name = "Variation1"
    end
  end
  ...
  if visible then
    ...
    if changed then 
      state.save_fx_chain = save_fx_chain
      if save_fx_chain then
        state.preset_name = "Chain1"
      else
        state.preset_name = "Variation1"
      end
    ...
    if r.ImGui_BeginChild(presets_ctx,"PresetsList",0,-1) then
      for name, preset_data in pairs(state.presets) do
        if preset_data.is_fx_chain then
          ...
          local tree_flags = r.ImGui_TreeNodeFlags_OpenOnArrow() | r.ImGui_TreeNodeFlags_DefaultOpen()
          local display_name = name .. " (FX Chain)"
          if r.ImGui_TreeNodeEx(presets_ctx, display_name, tree_flags) then
            
            r.ImGui_SameLine(presets_ctx, window_width - 115)
            if r.ImGui_Button(presets_ctx, "Load", 25, 20) then
              loadPreset(name)
            end
            r.ImGui_SameLine(presets_ctx)
            if r.ImGui_Button(presets_ctx, "R", 25, 20) then
            ...
            if preset_data.variations then
              for var_name, _ in pairs(preset_data.variations) do
                ...
                r.ImGui_Text(presets_ctx, var_name)
                r.ImGui_SameLine(presets_ctx, window_width - 115)
                if r.ImGui_Button(presets_ctx, "Load", 25, 20) then
                ...
                if r.ImGui_Button(presets_ctx, "R", 25, 20) then
                  state.show_preset_rename = true
                  state.rename_preset_name = var_name
                  state.selected_preset = name
                  state.selected_variation = var_name
                end
                r.ImGui_SameLine(presets_ctx)
                if r.ImGui_Button(presets_ctx, "X", 25, 20) then
                  if preset_data.variations then
          ...
          local btn_width = window_width - 115
          if r.ImGui_Button(presets_ctx, name, btn_width, 25) then
            loadPreset(name)
            state.selected_preset = name
          end
    ...
    if r.ImGui_BeginPopupModal(presets_ctx,"Rename Preset",nil,r.ImGui_WindowFlags_AlwaysAutoResize()) then
      ...
      if r.ImGui_Button(presets_ctx,"OK",120,0) then
        if state.selected_variation then
          local preset_data = state.presets[state.selected_preset]
          if preset_data and preset_data.variations and preset_data.variations[state.selected_variation] then
            preset_data.variations[state.rename_preset_name] = preset_data.variations[state.selected_variation]
            preset_data.variations[state.selected_variation] = nil
          end
          state.selected_variation = nil
        else
...
function drawCollapsedSection(section_name, content_func, width)
  local header_font = getStyleFont("header")
  
  if state.section_collapsed[section_name] then
    r.ImGui_PushStyleVar(ctx, r.ImGui_StyleVar_ButtonTextAlign(), 0.5, 0.0)
    local vertical_name = ""
    for i = 1, #section_name do
      vertical_name = vertical_name .. section_name:sub(i, i):upper()
      if i < #section_name then vertical_name = vertical_name .. "\n" end
    end
    
    if header_font then
      r.ImGui_PushFont(ctx, header_font)
      if r.ImGui_Button(ctx, vertical_name, 32, 340) then
        state.section_collapsed[section_name] = false
        scheduleSave()
      end
      r.ImGui_PopFont(ctx)
    else
    ...
    if header_font then
      r.ImGui_PushFont(ctx, header_font)
      local button_text = section_name:upper()
      if r.ImGui_Button(ctx, button_text, width or -1, 32) then
        state.section_collapsed[section_name] = true
        scheduleSave()
      end
      r.ImGui_PopFont(ctx)
    else
...
local changed,new_nav_mode=r.ImGui_Combo(ctx,"##navmode",state.navigation_mode,table.concat(navigation_modes,"\0").."\0")
if changed then 
...
elseif state.navigation_mode == 2 then
local changed,new_figures_mode=r.ImGui_Combo(ctx,"##figuresmode",state.figures_mode,table.concat(figures_modes,"\0").."\0")
if changed then 
...
if r.ImGui_Button(ctx,"Morph 1")then
captureToMorph(1)
end
r.ImGui_SameLine(ctx)
if r.ImGui_Button(ctx,"Morph 2")then
captureToMorph(2)
end
r.ImGui_SameLine(ctx)
...
local changed,new_amount=r.ImGui_SliderDouble(ctx,"Morph",state.morph_amount or 0,0.0,1.0)
if changed then
...
if r.ImGui_Button(ctx,"Auto JSFX")then
if state.jsfx_automation_enabled then
if state.jsfx_automation_index >= 0 then
r.TrackFX_Delete(state.track, state.jsfx_automation_index)
end
...
if r.ImGui_Button(ctx,"Show Env") and state.jsfx_automation_enabled and state.jsfx_automation_index >= 0 then
local x_env = r.GetFXEnvelope(state.track, state.jsfx_automation_index, 0, true)
local y_env = r.GetFXEnvelope(state.track, state.jsfx_automation_index, 1, true)
if x_env and y_env then
if not state.env_visible then
r.TrackFX_Show(state.track, state.jsfx_automation_index, 3)
state.env_visible = true
else
r.Main_OnCommand(40406, 0)
state.env_visible = false
end
end
end
end
...
function drawMode()
if r.ImGui_Button(ctx,state.pad_mode==0 and "Single [X]" or "Single",128,22)then
state.pad_mode=0
scheduleSave()
end
if r.ImGui_Button(ctx,state.pad_mode==1 and "Granular [X]" or "Granular",128,22)then
state.pad_mode=1
if not state.granular_grains or #state.granular_grains==0 then
initializeGranularGrid()
end
...
r.ImGui_SameLine(ctx)
local changed,new_bypass=r.ImGui_SliderDouble(ctx,"##bypass",state.random_bypass_percentage*100,0.0,100.0,"%.0f%%")
if changed then 
...
if r.ImGui_Button(ctx,fx_name_vertical,22,220)then
state.fx_collapsed[fx_id]=false
end
r.ImGui_PopStyleVar(ctx)
...
if r.ImGui_Button(ctx,"UI",22,22)then
local actual_fx_id = fx_data.actual_fx_id or fx_id
r.TrackFX_Show(state.track,actual_fx_id,3)
end
else
if r.ImGui_Button(ctx,fx_data.name,fx_width-88,22)then
local actual_fx_id = fx_data.actual_fx_id or fx_id
r.TrackFX_Show(state.track,actual_fx_id,3)
end
r.ImGui_SameLine(ctx)
            if r.ImGui_Button(ctx,"[-]",22,22)then
              state.fx_collapsed[fx_id]=true
            end
            r.ImGui_SameLine(ctx)
            local enabled=fx_data.enabled
...
r.ImGui_SetTooltip(ctx,param_data.name..": "..string.format("%.3f (Base: %.3f, Range: %.1f)",param_data.current_value,param_data.base_value,range)..xy_text..invert_text)
end
end
...
function drawHorizontalLayout()
  local section_width = 188
  if not state.section_collapsed.navigation then section_width = 188 else section_width = 32 end
  if r.ImGui_BeginChild(ctx, "Navigation", section_width, 0) then
    drawCollapsedSection("navigation", drawNavigation, 188)
    r.ImGui_EndChild(ctx)
    r.ImGui_SameLine(ctx)
    r.ImGui_Dummy(ctx, 0, 0)
    r.ImGui_SameLine(ctx)
  ...
  section_width = 128
  if state.section_collapsed.mode then section_width = 32 end
  if r.ImGui_BeginChild(ctx, "Mode", section_width, 0) then
    drawCollapsedSection("mode", drawMode, 128)
    r.ImGui_EndChild(ctx)
    r.ImGui_SameLine(ctx)
    r.ImGui_Dummy(ctx, 0, 0)
    r.ImGui_SameLine(ctx)
  ...
  section_width = 298
  if state.section_collapsed.pad then section_width = 32 end
  if r.ImGui_BeginChild(ctx, "PadXY", section_width, 0) then
    drawCollapsedSection("pad", drawPadSection, 298)
    r.ImGui_EndChild(ctx)
    r.ImGui_SameLine(ctx)
    r.ImGui_Dummy(ctx, 0, 0)
    r.ImGui_SameLine(ctx)
  ...
  section_width = 128
  if state.section_collapsed.randomizer then section_width = 32 end
  if r.ImGui_BeginChild(ctx, "Randomizer", section_width, 0) then
    drawCollapsedSection("randomizer", drawRandomizer, 128)
    r.ImGui_EndChild(ctx)
    r.ImGui_SameLine(ctx)
    r.ImGui_Dummy(ctx, 0, 0)
    r.ImGui_SameLine(ctx)
  ...
  local fx_width = 0
  if not state.section_collapsed.fx then fx_width = 0 else fx_width = 32 end
  if r.ImGui_BeginChild(ctx, "FX", fx_width, 0) then
    drawCollapsedSection("fx", drawFXSection, -1)
    r.ImGui_EndChild(ctx)
  end
end
