...
function UI.drawSoundGenerator()
	...
	if sg.waveform < 4 then
		...
		local log_value = math.log(sg.frequency)
		local normalized = (log_value - log_min) / (log_max - log_min)
		local changed, new_normalized = UI.r.ImGui_SliderDouble(UI.ctx, "Frequency##sg", normalized, 0, 1, "%.1f Hz", 1.0)
		if changed then
			local new_log_value = log_min + new_normalized * (log_max - log_min)
			sg.frequency = math.exp(new_log_value)
			UI.soundgen.updateJSFXParams()
		end
		if UI.r.ImGui_IsItemHovered(UI.ctx) then
			local mouse_wheel = UI.r.ImGui_GetMouseWheel(UI.ctx)
			if mouse_wheel ~= 0 then
				local new_log_value = log_value + mouse_wheel * 0.05
				new_log_value = math.max(log_min, math.min(log_max, new_log_value))
				sg.frequency = math.exp(new_log_value)
				UI.soundgen.updateJSFXParams()
			end
		end
		local freq_text = string.format("%.1f Hz", sg.frequency)
		local text_size = UI.r.ImGui_CalcTextSize(UI.ctx, freq_text)
		local item_min_x, item_min_y = UI.r.ImGui_GetItemRectMin(UI.ctx)
		local item_max_x, item_max_y = UI.r.ImGui_GetItemRectMax(UI.ctx)
		local text_x = item_min_x + (item_max_x - item_min_x - text_size) / 2
		local text_y = item_min_y + (item_max_y - item_min_y - 14) / 2
		local draw_list = UI.r.ImGui_GetWindowDrawList(UI.ctx)
		UI.r.ImGui_DrawList_AddText(draw_list, text_x, text_y, 0xFFFFFFFF, freq_text)
...
function UI.drawInterface()
	...
	if visible then
		...
		local settings_button_size = UI.header_font_size + 6
		local buttons_width = close_button_size + settings_button_size + UI.item_spacing_x
		local settings_x = UI.r.ImGui_GetWindowWidth(UI.ctx) - buttons_width - UI.window_padding_x
		UI.r.ImGui_SetCursorPosX(UI.ctx, settings_x)
		if UI.r.ImGui_Button(UI.ctx, "⚙", settings_button_size, settings_button_size) then
			UI.core.state.show_settings_window = not UI.core.state.show_settings_window
		end
		if UI.r.ImGui_IsItemHovered(UI.ctx) then
			UI.r.ImGui_SetTooltip(UI.ctx, "Settings")
		end
...
function UI.drawSettingsWindow()
	...
	if visible then
		...
		if header_font and UI.r.ImGui_ValidatePtr(header_font, "ImGui_Font*") then
			UI.r.ImGui_PushFont(UI.settings_ctx, header_font, 0)
			UI.r.ImGui_Text(UI.settings_ctx, "ULTRA RANDOM")
			UI.r.ImGui_PopFont(UI.settings_ctx)
		else
			UI.r.ImGui_Text(UI.settings_ctx, "Ultra Random:")
		end
		...
		local config = UI.core.state.ultra_random_config
		local changed, val = UI.r.ImGui_Checkbox(UI.settings_ctx, "Randomize Base Values", config.randomize_base)
		if changed then
			config.randomize_base = val
			UI.persistence.scheduleSave()
		end

		changed, val = UI.r.ImGui_Checkbox(UI.settings_ctx, "Randomize Ranges", config.randomize_range)
		if changed then
			config.randomize_range = val
			UI.persistence.scheduleSave()
		end

		changed, val = UI.r.ImGui_Checkbox(UI.settings_ctx, "Randomize FX Order", config.randomize_fx_order)
		if changed then
			config.randomize_fx_order = val
			UI.persistence.scheduleSave()
		end

		changed, val = UI.r.ImGui_Checkbox(UI.settings_ctx, "Randomize Bypass", config.randomize_bypass)
		if changed then
			config.randomize_bypass = val
			UI.persistence.scheduleSave()
		end

		changed, val = UI.r.ImGui_Checkbox(UI.settings_ctx, "Randomize XY Assignments", config.randomize_xy)
		if changed then
			config.randomize_xy = val
			UI.persistence.scheduleSave()
		end
