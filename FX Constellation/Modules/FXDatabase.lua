...
function FXDatabase.parseFXFolders()
	local categories = {
		...
		{name = "CP FX Constellation", type = "builtin", collapsed = false},
...
function FXDatabase.getBundledJSFX()
	return {
		{name = "CP FX Constellation/CP_FC_Filter.jsfx", display_name = "CP - Filter", type = "JS", instrument = false, favorite = false, bundled = true},
		{name = "CP FX Constellation/CP_FC_Distortion.jsfx", display_name = "CP - Distortion", type = "JS", instrument = false, favorite = false, bundled = true},
		{name = "CP FX Constellation/CP_FC_Bitcrusher.jsfx", display_name = "CP - Bitcrusher", type = "JS", instrument = false, favorite = false, bundled = true},
		{name = "CP FX Constellation/CP_FC_Gain.jsfx", display_name = "CP - Gain", type = "JS", instrument = false, favorite = false, bundled = true},
		{name = "CP FX Constellation/CP_FC_RingMod.jsfx", display_name = "CP - Ring Mod", type = "JS", instrument = false, favorite = false, bundled = true},
		{name = "CP FX Constellation/CP_FC_Tremolo.jsfx", display_name = "CP - Tremolo", type = "JS", instrument = false, favorite = false, bundled = true},
		{name = "CP FX Constellation/CP_FC_Chorus.jsfx", display_name = "CP - Chorus", type = "JS", instrument = false, favorite = false, bundled = true},
		{name = "CP FX Constellation/CP_FC_Phaser.jsfx", display_name = "CP - Phaser", type = "JS", instrument = false, favorite = false, bundled = true},
		{name = "CP FX Constellation/CP_FC_Flanger.jsfx", display_name = "CP - Flanger", type = "JS", instrument = false, favorite = false, bundled = true},
		{name = "CP FX Constellation/CP_FC_Delay.jsfx", display_name = "CP - Delay", type = "JS", instrument = false, favorite = false, bundled = true},
		{name = "CP FX Constellation/CP_FC_Compressor.jsfx", display_name = "CP - Compressor", type = "JS", instrument = false, favorite = false, bundled = true},
		{name = "CP FX Constellation/CP_FC_Gate.jsfx", display_name = "CP - Gate", type = "JS", instrument = false, favorite = false, bundled = true},
	}
end

function FXDatabase.injectBundledJSFX()
	local bundled = FXDatabase.getBundledJSFX()
	local effects_dir = FXDatabase.r.GetResourcePath() .. "/Effects/"

	for _, jsfx in ipairs(bundled) do
		-- Only inject if the file actually exists on disk
		local file_path = effects_dir .. jsfx.name
		local f = io.open(file_path, "r")
		if f then
			f:close()

			-- Check if already in the plugins list
			local found = false
			for _, plugin in ipairs(FXDatabase.plugins) do
				if plugin.name == jsfx.name then
					plugin.bundled = true
					found = true
					break
				end
			...
			if not found then
				...
				if FXDatabase.favorites[jsfx.name] then
					jsfx.favorite = true
				end
				table.insert(FXDatabase.plugins, jsfx)
			end
		end
	end
end
...
function FXDatabase.getPluginsByCategory(category_name)
	...
	elseif category_name == "CP FX Constellation" then
...
function FXDatabase.filterBundled()
	local results = {}
	for _, plugin in ipairs(FXDatabase.plugins) do
