...
function FXManager.ultraRandom()
	local config = FXManager.core.state.ultra_random_config
	if config.randomize_base then
		FXManager.randomizeAllBases()
	end
	if config.randomize_range then
		FXManager.globalRandomRanges()
	end
	if config.randomize_fx_order then
		FXManager.randomizeFXOrder()
	end
	if config.randomize_bypass then
		FXManager.randomBypassFX()
	end
	if config.randomize_xy then
		FXManager.globalRandomInvert()
	end
	if config.randomize_n then
...
function FXManager.randomizeFXOrder()
	...
	local start_idx = 0
	local _, first_fx_name = FXManager.r.TrackFX_GetFXName(FXManager.core.state.track, 0, "")
	if first_fx_name:find("Sound Generator") then
		start_idx = 1
	end

	for i = fx_count - 1, start_idx + 1, -1 do
		...
		if not fx_name_i:find("FX Constellation Bridge") and not fx_name_i:find("Sound Generator") then
			local j = math.random(start_idx, i - 1)
