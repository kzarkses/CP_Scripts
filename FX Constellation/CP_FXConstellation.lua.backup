		...
		if total_weights[fx_id] and total_weights[fx_id] > 0 then
			for param_id, param_data in pairs(fx_data.params) do
				if param_data.selected and weighted_param_values[fx_id][param_id] then
					...
					local denormalized_value = denormalizeParamValue(final_value, param_data.min_val, param_data.max_val)
					r.TrackFX_SetParam(state.track, actual_fx_id, param_id, denormalized_value)
...
function saveGranularSet(name)
	...
	if not state.presets[current_preset] then return end
	
	if not state.presets[current_preset].granular_sets then
		state.presets[current_preset].granular_sets = {}
	end
	
	state.presets[current_preset].granular_sets[name] = {
		grid_size = state.granular_grid_size,
		grains = {}
	}
	for i, grain in ipairs(state.granular_grains) do
		state.presets[current_preset].granular_sets[name].grains[i] = {
			x = grain.x,
			y = grain.y,
			param_values = {}
		}
		for fx_id, params in pairs(grain.param_values) do
			state.presets[current_preset].granular_sets[name].grains[i].param_values[fx_id] = {}
			for param_id, value in pairs(params) do
				state.presets[current_preset].granular_sets[name].grains[i].param_values[fx_id][param_id] = value
			end
		end
	end
	state.granular_set_name = GetNextGranularSetName()
	schedulePresetSave()
end

function loadGranularSet(name)
	local current_preset = state.current_loaded_preset
	if current_preset == "" or not state.presets[current_preset] or not state.presets[current_preset].granular_sets then
		return
	end
	
	local set_data = state.presets[current_preset].granular_sets[name]
...
function deleteGranularSet(name)
	local current_preset = state.current_loaded_preset
	if current_preset ~= "" and state.presets[current_preset] and state.presets[current_preset].granular_sets and state.presets[current_preset].granular_sets[name] then
		state.presets[current_preset].granular_sets[name] = nil
		if not next(state.presets[current_preset].granular_sets) then
			state.presets[current_preset].granular_sets = {}
		end
		schedulePresetSave()
	end
...
function saveSnapshot(name)
	...
	local current_preset = state.current_loaded_preset
	if current_preset == "" then
		r.ShowMessageBox("Please load a preset first before saving snapshots.", "FX Constellation", 0)
		return
	end
	
	if not state.presets[current_preset] then return end
	
	if not state.presets[current_preset].snapshots then
		state.presets[current_preset].snapshots = {}
	end
	...
	state.presets[current_preset].snapshots[name] = snapshot
...
function loadSnapshot(name)
	...
	local current_preset = state.current_loaded_preset
	if current_preset == "" or not state.presets[current_preset] or not state.presets[current_preset].snapshots or not state.presets[current_preset].snapshots[name] then
		return
	end
	
	local snapshot = state.presets[current_preset].snapshots[name]
	...
	for fx_id, fx_data in pairs(state.fx_data) do
		for param_id, param_data in pairs(fx_data.params) do
			...
			if saved_param then
				...
				local actual_fx_id = fx_data.actual_fx_id or fx_id
				local denormalized_value = denormalizeParamValue(param_data.base_value, param_data.min_val, param_data.max_val)
				r.TrackFX_SetParam(state.track, actual_fx_id, param_id, denormalized_value)
...
function deleteSnapshot(name)
	local current_preset = state.current_loaded_preset
	if current_preset ~= "" and state.presets[current_preset] and state.presets[current_preset].snapshots and state.presets[current_preset].snapshots[name] then
		state.presets[current_preset].snapshots[name] = nil
		if not next(state.presets[current_preset].snapshots) then
			state.presets[current_preset].snapshots = {}
		end
		schedulePresetSave()
	end
...
function EnsureDataDirectory()
	...
	for i, part in ipairs(path_parts) do
		...
		if not r.file_exists(current_path) then
			r.RecursiveCreateDirectory(current_path, 0)
		end
	end
end

function SavePresetsToFile()
	if not next(state.presets) then return end
	EnsureDataDirectory()
	local file = io.open(presets_file, "w")
	if file then
		file:write(serialize(state.presets))
		file:close()
	end
end
	...
	if r.file_exists(presets_file) then
		...
		if file then
			...
			if content and content ~= "" then
				state.presets = deserialize(content) or {}
			end
		end
	end
end

function SaveGranularSetsToFile()
	if not next(state.granular_sets) then return end
	EnsureDataDirectory()
	local file = io.open(granular_sets_file, "w")
	if file then
		file:write(serialize(state.granular_sets))
		file:close()
	end
end

function LoadGranularSetsFromFile()
	if r.file_exists(granular_sets_file) then
		...
		if file then
			...
			file:close()
			if content and content ~= "" then
				state.granular_sets = deserialize(content) or {}
			end
		end
	end
end
...
function saveSettings()
	...
	if save_flags.presets then
		SavePresetsToFile()
	end

	if save_flags.granular_sets then
		SaveGranularSetsToFile()
	end
...
function normalizeParamValue(value, min_val, max_val)
	if min_val == 0 and max_val == 1 then
		return value
	end
...
function denormalizeParamValue(normalized_value, min_val, max_val)
	if min_val == 0 and max_val == 1 then
		...
		if not fx_name:find("FX Constellation Bridge") then
			...
			for param = 0, param_count - 1 do
				...
				if not shouldFilterParam(param_name) then
					local value, min_val, max_val = r.TrackFX_GetParamEx(state.track, fx, param)
					if not min_val or not max_val then
						min_val = 0
						max_val = 1
						value = r.TrackFX_GetParam(state.track, fx, param)
					end
					local normalized_value = normalizeParamValue(value, min_val, max_val)
	...
	if current_fx_count ~= state.last_fx_count or current_signature ~= state.last_fx_signature then
		...
		for fx_id, fx_data in pairs(state.fx_data) do
			...
			for param_id, param_data in pairs(fx_data.params) do
				...
				local raw_value = r.TrackFX_GetParam(state.track, actual_fx_param_id, param_id)
				local current_value = normalizeParamValue(raw_value, param_data.min_val, param_data.max_val)
...
function selectAllContinuousParams(params, selected)
	for _, param in pairs(params) do
		if not param.step_count or param.step_count == 0 or param.step_count > 10 then
...
function randomizeBaseValues(params, fx_id)
	...
	for param_id, param_data in pairs(params) do
		if param_data.selected then
			...
			local denormalized_value = denormalizeParamValue(new_base, param_data.min_val, param_data.max_val)
			r.TrackFX_SetParam(state.track, actual_fx_id, param_id, denormalized_value)
...
function updateParamBaseValue(fx_id, param_id, new_value)
	...
	if param_data then
		...
		local denormalized_value = denormalizeParamValue(new_value, param_data.min_val, param_data.max_val)
		r.TrackFX_SetParam(state.track, actual_fx_id, param_id, denormalized_value)
...
function detectParamSteps(fx_id, param_id)
	...
	local _, min_val, max_val = r.TrackFX_GetParamEx(state.track, actual_fx_id, param_id)
		...
		if preset_a and preset_b then
			...
			for param_id, param_data in pairs(fx_data.params) do
				...
				if value_a and value_b then
					...
					local actual_fx_id = fx_data.actual_fx_id or fx_id
					local denormalized_value = denormalizeParamValue(morphed_value, param_data.min_val, param_data.max_val)
					r.TrackFX_SetParam(state.track, actual_fx_id, param_id, denormalized_value)
				...
				for saved_param_id, param_preset in pairs(fx_preset.params or {}) do
					for current_param_id, param_data in pairs(fx_data.params) do
						if param_data.name == param_preset.name then
							...
							local denormalized_value = denormalizeParamValue(param_preset.current_value, param_data.min_val, param_data.max_val)
							r.TrackFX_SetParam(state.track, actual_fx_id, current_param_id, denormalized_value)
...
function drawNavigation()
	...
	elseif state.navigation_mode == 2 then
		...
		if changed then
			local current_angle = state.figures_time * state.figures_speed * 2 * math.pi
			local current_progress = (current_angle / (2 * math.pi)) % 1
			state.figures_time = (current_progress / new_speed)
...
function drawRandomizer()
	...
	r.ImGui_Dummy(ctx, 0, 0)
	if r.ImGui_Button(ctx, "Parameters", content_width) then
		globalRandomSelect()
		saveTrackSelection()
	end
						...
						if r.ImGui_Button(ctx, "All##" .. fx_id, item_width) then
							...
							if r.ImGui_BeginTable(ctx, "params" .. fx_id, 6, table_flags) then
								...
								for param_id, param_data in pairs(fx_data.params) do
									...
									local real_current = denormalizeParamValue(param_data.current_value, real_min, real_max)
									local real_base = denormalizeParamValue(param_data.base_value, real_min, real_max)
