...
local Utils = dofile(script_path .. "Core/utils.lua")
local SaveManager = dofile(script_path .. "Core/save_manager.lua")
local State = dofile(script_path .. "Core/state.lua")
local PresetManager = dofile(script_path .. "Persistence/preset_manager.lua")
local GranularEngine = dofile(script_path .. "Gesture/granular_engine.lua")
...
function saveGranularSet(name)
  local grains = GranularEngine.GetGrains()
  if name == "" or #grains == 0 then return end
...
function SavePresetsToFile()
  PresetManager.SavePresetsToFile(Utils)
end

function LoadPresetsFromFile()
  PresetManager.LoadPresetsFromFile(Utils)
  state.presets = PresetManager.GetPresets()
end
...
function InitializeModules()
  PresetManager.Initialize(Utils)
  GranularEngine.Initialize(Utils)
  
  SaveManager.RegisterModule("state", State)
  SaveManager.RegisterModule("presets", PresetManager)
  SaveManager.RegisterModule("granular", GranularEngine)
end
...
function updateAutomationFromJSFX()
  ...
  if state.pad_mode == 1 then
    GranularEngine.ApplyGesture(jsfx_x, jsfx_y, state)
  else
  ...
  if state.navigation_mode == 1 then
    if state.random_walk_active then
      ...
      if state.random_walk_control_points and state.random_walk_control_points.p0 then
        ...
        if state.pad_mode == 1 then
          GranularEngine.ApplyGesture(state.gesture_x, state.gesture_y, state)
        else
...
function checkSave()
  SaveManager.CheckSave(Utils)
  
  if save_flags.track_selections then
    saveTrackSelectionOnly()
...
function saveTrackSelectionOnly()
