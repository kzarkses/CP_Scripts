...
local script_name = "CP_PitchShiftSelector"
...
local state = {
    ...
    current_submode = 0,
    window_open = true,
    algorithms_column_height = 300,
    stretch_settings_height = 80,
    rrreeeaaa_params = {
        syn = 0,
        ano = 0,
        fft = 0,
...
function DecomposeSubmode(mode, submode)
    if mode == 0 then
...
function GetCurrentModeFromSelection()
    ...
    if mode == 14 then
        state.rrreeeaaa_params.syn = submode & bitmask.syn
        state.rrreeeaaa_params.ano = submode & bitmask.ano
        state.rrreeeaaa_params.fft = submode & bitmask.fft
        state.rrreeeaaa_params.anw = submode & bitmask.anw
        state.rrreeeaaa_params.syw = submode & bitmask.syw
    elseif mode == 15 then
        state.rearearea_params.rnd = submode & bitmask.rnd
        state.rearearea_params.fdm = submode & bitmask.fdm
        state.rearearea_params.shp = submode & bitmask.shp
        state.rearearea_params.snc = submode & bitmask.snc
        state.rearearea_params.snc_checkbox = snc_to_checkbox[state.rearearea_params.snc]
    elseif algorithm_params[mode] then
        DecomposeSubmode(mode, submode)
    end
    
    state.current_mode = mode
    state.current_submode = submode
    return true
end

function ComposeSubmode(mode)
    if mode == 0 then
...
function GetStretchMarkerSettings()
    if r.SNM_GetDoubleConfigVar then
        local fade_from_reaper = r.SNM_GetDoubleConfigVar("smfadesize", -1)
        if fade_from_reaper >= 0 then
            config.stretch_fade_size = fade_from_reaper * 1000
        end
    end
end

function ApplyStretchMarkerSettings()
    if r.SNM_SetDoubleConfigVar then
        r.SNM_SetDoubleConfigVar("smfadesize", config.stretch_fade_size / 1000)
    end
    
    local action_id = stretch_mode_actions[config.stretch_mode + 1]
    if action_id and action_id > 0 then
        r.Main_OnCommand(action_id, 0)
    end
end

function ApplyBasicPitchMode(mode_idx, submode_idx)
    ...
    state.current_submode = submode_idx
end

function ApplyParametricPitchMode(mode_idx)
    local item_count = r.CountSelectedMediaItems(0)
    if item_count == 0 then return end
...
function ApplyRrreeeaaaMode()
    local item_count = r.CountSelectedMediaItems(0)
    if item_count == 0 then return end
    
    r.Undo_BeginBlock()
    
    local submode = state.rrreeeaaa_params.syn + state.rrreeeaaa_params.ano + state.rrreeeaaa_params.fft + 
                    state.rrreeeaaa_params.anw + state.rrreeeaaa_params.syw
    ...
    state.current_mode = 14
    state.current_submode = submode
end

function ApplyReaReaReaMode()
    local item_count = r.CountSelectedMediaItems(0)
    if item_count == 0 then return end
    
    r.Undo_BeginBlock()
    
    local submode = state.rearearea_params.rnd + state.rearearea_params.fdm + 
                   state.rearearea_params.shp + state.rearearea_params.snc
...
function RandomizeAlgorithm()
    ...
    if algo.index == 14 then
        state.rrreeeaaa_params.syn = slider_to_syn[math.random(3, 10)]
        state.rrreeeaaa_params.fft = slider_to_fft[math.random(0, 3)]
        state.rrreeeaaa_params.ano = slider_to_ano[math.random(0, 3)]
        state.rrreeeaaa_params.anw = slider_to_anw[math.random(0, 3)]
        state.rrreeeaaa_params.syw = slider_to_syw[math.random(0, 3)]
        ApplyRrreeeaaaMode()
    elseif algo.index == 15 then
        state.rearearea_params.rnd = math.random(0, 15)
        state.rearearea_params.fdm = slider_to_fdm[math.random(0, 63)]
        state.rearearea_params.shp = slider_to_shp[math.random(0, 2)]
        -- FIX: Utiliser une approche plus sûre pour snc
        local snc_bool = math.random(0, 1) == 1
        state.rearearea_params.snc = checkbox_to_snc[snc_bool]
        state.rearearea_params.snc_checkbox = snc_bool
        ApplyReaReaReaMode()
...
function RandomizeSubmode()
    ...
    if state.current_mode == 14 then
        ...
        state.rrreeeaaa_params.syw = slider_to_syw[math.random(0, 3)]
        ApplyRrreeeaaaMode()
    elseif state.current_mode == 15 then
        state.rearearea_params.rnd = math.random(0, 15)
        state.rearearea_params.fdm = slider_to_fdm[math.random(0, 63)]
        state.rearearea_params.shp = slider_to_shp[math.random(0, 2)]
        -- FIX: Approche plus sûre pour snc
        local snc_bool = math.random(0, 1) == 1
        state.rearearea_params.snc = checkbox_to_snc[snc_bool]
        state.rearearea_params.snc_checkbox = snc_bool
        ApplyReaReaReaMode()
    elseif algorithm_params[state.current_mode] then
        if state.current_mode == 0 then
            algorithm_params[0].channel = math.random(0, 2)
            algorithm_params[0].quality = math.random(0, 2)
        elseif state.current_mode == 2 then
            algorithm_params[2].ms_window = math.random(0, 11)
            algorithm_params[2].fade_percent = math.random(0, 3)
        elseif state.current_mode == 6 or state.current_mode == 9 then
            algorithm_params[state.current_mode].preserve_formants = math.random(0, 7)
            algorithm_params[state.current_mode].synchronized = math.random(0, 1) == 1
            algorithm_params[state.current_mode].mid_side = math.random(0, 1) == 1
            algorithm_params[state.current_mode].channel = math.random(0, 2)
        elseif state.current_mode == 7 or state.current_mode == 10 then
            algorithm_params[state.current_mode].synchronized = math.random(0, 1) == 1
            algorithm_params[state.current_mode].mid_side = math.random(0, 1) == 1
            algorithm_params[state.current_mode].channel = math.random(0, 2)
        elseif state.current_mode == 8 or state.current_mode == 11 then
            algorithm_params[state.current_mode].mode = math.random(0, 1)
            algorithm_params[state.current_mode].mid_side = math.random(0, 1) == 1
            algorithm_params[state.current_mode].channel = math.random(0, 2)
        end
        ApplyParametricPitchMode(state.current_mode)
    end
end
...
function RenderRrreeeaaaControls()
    local changed = false
    local rv
    
    local syn_slider = syn_to_slider[state.rrreeeaaa_params.syn]
    rv, syn_slider = r.ImGui_SliderInt(ctx, "Synthesis", syn_slider, 3, 10, "%dx")
    if rv then
        state.rrreeeaaa_params.syn = slider_to_syn[syn_slider]
        changed = true
    end
    
    local fft_slider = fft_to_slider[state.rrreeeaaa_params.fft]
    rv, fft_slider = r.ImGui_SliderInt(ctx, "FFT", fft_slider, 0, 3, fft_names[fft_slider])
    if rv then
        state.rrreeeaaa_params.fft = slider_to_fft[fft_slider]
        changed = true
    end
    
    local ano_slider = ano_to_slider[state.rrreeeaaa_params.ano]
    rv, ano_slider = r.ImGui_SliderInt(ctx, "Analysis Offset", ano_slider, 0, 3, ano_names[ano_slider])
    if rv then
        state.rrreeeaaa_params.ano = slider_to_ano[ano_slider]
        changed = true
    end
    
    local anw_slider = anw_to_slider[state.rrreeeaaa_params.anw]
    rv, anw_slider = r.ImGui_SliderInt(ctx, "Analysis Window", anw_slider, 0, 3, anw_names[anw_slider])
    if rv then
        state.rrreeeaaa_params.anw = slider_to_anw[anw_slider]
        changed = true
    end
    
    local syw_slider = syw_to_slider[state.rrreeeaaa_params.syw]
    rv, syw_slider = r.ImGui_SliderInt(ctx, "Synthesis Window", syw_slider, 0, 3, syw_names[syw_slider])
    if rv then
        state.rrreeeaaa_params.syw = slider_to_syw[syw_slider]
        changed = true
    end
...
function RenderReaReaReaControls()
    local changed = false
    local rv
    
    rv, state.rearearea_params.snc_checkbox = r.ImGui_Checkbox(ctx, "Tempo Synced", state.rearearea_params.snc_checkbox)
    if rv then
        state.rearearea_params.snc = checkbox_to_snc[state.rearearea_params.snc_checkbox]
        changed = true
    end
    
    if state.rearearea_params.snc_checkbox then
        local fds_slider = fds_to_slider[state.rearearea_params.fdm]
        rv, fds_slider = r.ImGui_SliderInt(ctx, "Fade", fds_slider, 0, 23, fds_names[fds_slider])
        if rv then
            state.rearearea_params.fdm = slider_to_fds[fds_slider]
            changed = true
        end
    else
        local fdm_slider = fdm_to_slider[state.rearearea_params.fdm]
        rv, fdm_slider = r.ImGui_SliderInt(ctx, "Fade", fdm_slider, 0, 63, fdm_names[fdm_slider])
        if rv then
            state.rearearea_params.fdm = slider_to_fdm[fdm_slider]
            changed = true
        end
    end
    
    rv, state.rearearea_params.rnd = r.ImGui_SliderInt(ctx, "Randomize", state.rearearea_params.rnd, 0, 15, rnd_names[state.rearearea_params.rnd])
    if rv then
        changed = true
    end
    
    local shp_slider = shp_to_slider[state.rearearea_params.shp]
    rv, shp_slider = r.ImGui_SliderInt(ctx, "Shape", shp_slider, 0, 2, shp_names[shp_slider])
    if rv then
        state.rearearea_params.shp = slider_to_shp[shp_slider]
        changed = true
    end
...
function RenderAlgorithmControls(mode)
...
function MainLoop()
    ...
    if visible then
        ...
        local button_width = ((content_width - 2 * item_spacing_x) / 3)
        
        if r.ImGui_Button(ctx, "Random Algorithm", button_width) then
            RandomizeAlgorithm()
        end
        
        r.ImGui_SameLine(ctx)
        
        if r.ImGui_Button(ctx, "Random Submode", button_width) then
            RandomizeSubmode()
        end
        
        r.ImGui_SameLine(ctx)
        
        if r.ImGui_Button(ctx, "Full Random", button_width) then
            RandomizeFull()
        end
        ...
        if r.ImGui_BeginChild(ctx, "main_layout", 0, 0) then
            ...
            local algorithms_start_y = r.ImGui_GetCursorPosY(ctx)
            ...
            if r.ImGui_BeginChild(ctx, "algorithms", left_column_width, 0) then
                ...
                for i, algo in ipairs(ALGORITHMS) do
                    local is_selected = (algo.index == state.current_mode)
                    
                    if is_selected then
                        r.ImGui_PushStyleColor(ctx, r.ImGui_Col_Button(), r.ImGui_GetStyleColor(ctx, r.ImGui_Col_ButtonActive()))
                    end
                    
                    if r.ImGui_Button(ctx, algo.name, button_width) then
                        if algo.index == 14 then
                            ApplyRrreeeaaaMode()
                        elseif algo.index == 15 then
                            ApplyReaReaReaMode()
                ...
                local algorithms_end_y = r.ImGui_GetCursorPosY(ctx)
                state.algorithms_column_height = algorithms_end_y - algorithms_start_y
            ...
            if r.ImGui_BeginChild(ctx, "right_panel", 0, 0) then
                local options_height = state.algorithms_column_height - state.stretch_settings_height - item_spacing_y
                
                if r.ImGui_BeginChild(ctx, "options", 0, options_height) then
                    ...
                    elseif algorithm_params[state.current_mode] then
                        RenderAlgorithmControls(state.current_mode)
                    else
                ...
                local stretch_start_y = r.ImGui_GetCursorPosY(ctx)
                
                r.ImGui_Text(ctx, "Stretch Marker Settings:")
                r.ImGui_Separator(ctx)
                ...
                rv, config.stretch_fade_size = r.ImGui_SliderDouble(ctx, "Fade Size (ms)", config.stretch_fade_size, 0.1, 100.0, "%.1f ms")
                if rv then
                    ApplyStretchMarkerSettings()
                end
                
                rv, config.stretch_mode = r.ImGui_SliderInt(ctx, "Mode", config.stretch_mode, 0, 4, stretch_mode_names[config.stretch_mode + 1])
                if rv then
                    ApplyStretchMarkerSettings()
                end
                
                local stretch_end_y = r.ImGui_GetCursorPosY(ctx)
                state.stretch_settings_height = stretch_end_y - stretch_start_y
