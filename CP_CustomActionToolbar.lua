...
local extname = "CP_CUSTOM_ACTION_TOOLBAR 4"
...
local settings = {
    ...
    left_edge_offset = 10,  -- Décalage par rapport au bord gauche
    right_edge_offset = 10, -- Décalage par rapport au bord droit
...
-- Create context
local ctx = r.ImGui_CreateContext('Custom Action Toolbar 4')
...
function DisplayToolbar()
    ...
    for i = 1, #settings.actions do
        if settings.actions[i].is_separator then
            total_width = total_width + (settings.actions[i].width or 10) + (i > 1 and settings.button_spacing or 0)
        else
    ...
    for i, action in ipairs(settings.actions) do
        ...
        if action.is_separator then
            -- Draw a separator
            r.ImGui_Dummy(ctx, action.width or 10, settings.button_height)
...
function ShowSettingsDialog()
    ...
    if position_open then
        ...
        r.ImGui_Text(ctx, "Position Mode:")
        
        local is_transport = settings.screen_edge_mode == "transport"
        rv = r.ImGui_RadioButton(ctx, "Follow Transport", is_transport)
        if rv and not is_transport then
            settings.screen_edge_mode = "transport"
        ...
        r.ImGui_SameLine(ctx)
        local is_left = settings.screen_edge_mode == "left"
        rv = r.ImGui_RadioButton(ctx, "Snap to Left Edge", is_left)
        if rv and not is_left then
            settings.screen_edge_mode = "left"
        ...
        r.ImGui_SameLine(ctx)
        local is_right = settings.screen_edge_mode == "right"
        rv = r.ImGui_RadioButton(ctx, "Snap to Right Edge", is_right)
        if rv and not is_right then
            settings.screen_edge_mode = "right"
            settings.overlay_enabled = false
            force_position_update = true
            first_position_set = false
        end
        ...
        if settings.screen_edge_mode == "transport" then
        ...
        elseif settings.screen_edge_mode == "left" then
            r.ImGui_Text(ctx, "Distance from left edge:")
            rv, changed = r.ImGui_SliderInt(ctx, "Left Offset", settings.left_edge_offset, 0, 500)
            if rv and changed ~= settings.left_edge_offset then
                settings.left_edge_offset = changed
                force_position_update = true
            end
            
            rv, changed = r.ImGui_SliderDouble(ctx, "Y position", settings.rel_pos_y, 0.0, 2000.0, "%.2f")
            if rv and changed ~= settings.rel_pos_y then
                settings.rel_pos_y = changed
                force_position_update = true
            end
        elseif settings.screen_edge_mode == "right" then
            r.ImGui_Text(ctx, "Distance from right edge:")
            rv, changed = r.ImGui_SliderInt(ctx, "Right Offset", settings.right_edge_offset, 0, 500)
            if rv and changed ~= settings.right_edge_offset then
                settings.right_edge_offset = changed
                force_position_update = true
            end
        ...
        rv, changed = r.ImGui_SliderInt(ctx, "Width", settings.widget_width, 20, 2000)
        ...
        rv, changed = r.ImGui_Checkbox(ctx, "Enable auto-hide", settings.auto_hide)
        if rv and changed ~= settings.auto_hide then
            settings.auto_hide = changed
        end
        
        r.ImGui_SetNextItemWidth(ctx, 200)
        rv, changed = r.ImGui_SliderInt(ctx, "Min window width for auto-hide", settings.min_window_width, 500, 3000)
        if rv and changed ~= settings.min_window_width then
            settings.min_window_width = changed
        end
        
        r.ImGui_SetNextItemWidth(ctx, 200)
        rv, changed = r.ImGui_SliderInt(ctx, "Min window height for auto-hide", settings.min_window_height, 100, 2000)
        if rv and changed ~= settings.min_window_height then
            settings.min_window_height = changed
        end
        ...
        if r.ImGui_IsItemHovered(ctx) then
            r.ImGui_BeginTooltip(ctx)
            r.ImGui_Text(ctx, "Toolbar will auto-hide when REAPER window is smaller than these dimensions")
            r.ImGui_EndTooltip(ctx)
        end
    end
    ...
    if actions_open then
        ...
        r.ImGui_SameLine(ctx)
        r.ImGui_PushStyleColor(ctx, r.ImGui_Col_Button(), 0x72B34CFF) -- Green button for separator
        if r.ImGui_Button(ctx, "Add Separator") then
            table.insert(settings.actions, {
                is_separator = true,
                width = 10 -- Default separator width
            })
        end
        r.ImGui_PopStyleColor(ctx, 2) -- Pop both button colors
            ...
            if r.ImGui_BeginChild(ctx, "Actions List", 0, available_height) then
                for i, action in ipairs(settings.actions) do
                    ...
                    if action.is_separator then
                        ...
                        local width_changed, new_width = r.ImGui_SliderInt(ctx, "##width" .. i, action.width or 10, 1, 100)
...
function DisplayToolbarWidget()
    ...
    if settings.screen_edge_mode == "transport" then
        ...
        if transport_hwnd then
            ...
            if retval then
                ...
                local target_x = RIGHT - settings.widget_width - settings.rel_pos_x
                local target_y = TOP + settings.rel_pos_y
    ...
    elseif settings.screen_edge_mode == "left" then
        ...
        if main_hwnd then
            ...
            if retval then
                ...
                local target_x = LEFT + settings.left_edge_offset
                local target_y = TOP + settings.rel_pos_y
    ...
    elseif settings.screen_edge_mode == "right" then
        ...
        if main_hwnd then
            ...
            if retval then
                if r.APIExists("ImGui_PointConvertNative") then
                    LEFT, TOP, RIGHT, BOT = r.ImGui_PointConvertNative(ctx, LEFT, TOP, RIGHT, BOT)
                end
                
                local target_x = RIGHT - settings.widget_width - settings.right_edge_offset
                local target_y = TOP + settings.rel_pos_y
    ...
    -- Start the window
    local visible, open = r.ImGui_Begin(ctx, 'Custom Action Toolbar', true, toolbar_flags)
    
    if visible then
        DisplayToolbar()
        
        -- Save position if in free mode (not snapped to any edge)
        if settings.screen_edge_mode ~= "transport" and 
           settings.screen_edge_mode ~= "left" and 
           settings.screen_edge_mode ~= "right" then
...
function MainLoop()
    ...
    if not ctx or r.ImGui_ValidatePtr(ctx, "ImGui_Context*") == false then
        ctx = r.ImGui_CreateContext('Custom Action Toolbar 4')
