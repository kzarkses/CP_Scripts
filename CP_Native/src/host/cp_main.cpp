...
static void register_all(reaper_plugin_info_t* rec) {
  ...
  REG(CP_LaneCount, "int\0\0\0Nombre de lanes MIDI que ce binaire sert.");
  ...
  REG(CP_SetLaunchQ, "void\0double\0beats\0Quantize de lancement en beats. 0 = agir tout de suite.");
  REG(CP_GetLaunchQ, "double\0\0\0");
  REG(CP_SetAudioRun, "void\0int\0on\0Une case audio de Lua sonne : l'horloge libre est le transport de la SESSION.");
  REG(CP_EngineBeat, "double\0\0\0Position de l'horloge sur laquelle le moteur travaille. Les cibles en attente sont sur cette ligne de temps.");
  REG(CP_ClockRunning, "bool\0\0\0Y a-t-il une horloge du tout.");
  REG(CP_LanesPanic, "void\0\0\0Arrete toutes les lanes et relache toutes leurs notes.");
  REG(CP_LanesDiag, "const char*\0\0\0Etat des lanes en une ligne.");
